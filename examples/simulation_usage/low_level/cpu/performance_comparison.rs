...
impl DetailedPerformanceMetrics {
    ...
    fn analyze_bottlenecks(&mut self) {
        if self.update_times.len() < 60 { // Need enough samples
            return;
        }
        ...
        if avg_force_time > avg_integration_time * 2.0 {
            ...
            self.optimization_suggestions.push("Consider spatial partitioning for neighbor search".to_string());
            self.optimization_suggestions.push("Implement force approximation algorithms".to_string());
        } else if avg_integration_time > avg_force_time * 1.5 {
            self.bottleneck_analysis = "Integration bottleneck".to_string();
            self.optimization_suggestions.push("Use SIMD instructions for integration".to_string());
            self.optimization_suggestions.push("Consider reduced precision integration".to_string());
        } else if avg_sync_time > total_compute_time {
            self.bottleneck_analysis = "Scene synchronization bottleneck".to_string();
            self.optimization_suggestions.push("Batch scene updates".to_string());
            self.optimization_suggestions.push("Use dirty flagging for unchanged objects".to_string());
        } else {
            self.bottleneck_analysis = "Balanced CPU performance".to_string();
            self.optimization_suggestions.push("Consider multi-threading for more particles".to_string());
        }
    }
...
trait Force {
    fn calculate(&self, particle: &AdvancedParticle, all_particles: &[AdvancedParticle], time: f32) -> Vector3<f32>;
    fn name(&self) -> &str;
}
...
impl Force for GravityForce {
    fn calculate(&self, _particle: &AdvancedParticle, _all_particles: &[AdvancedParticle], _time: f32) -> Vector3<f32> {
        self.strength
    }
...
impl Force for FlockingForce {
    fn calculate(&self, particle: &AdvancedParticle, all_particles: &[AdvancedParticle], _time: f32) -> Vector3<f32> {
...
impl LowLevelCPUSimulation {
    ...
    fn calculate_forces_multithreaded(&mut self) {
        ...
        for i in 0..self.particles.len() {
            ...
            for force in &self.forces {
                let applied_force = force.calculate(&self.particles[i], &particles_clone, self.time);
                self.particles[i].add_force(applied_force);
            }
        }
    }
...
impl Simulation for LowLevelCPUSimulation {
    fn initialize(&mut self, _scene: &mut Scene) {
        ...
        println!("Initialized {} particles with low-level CPU management", self.particles.len());
    ...
    fn render_ui(&mut self, ui: &Ui) {
        ...
        ui.window("Low-Level CPU Simulation")
            ...
            .build(|| {
                ...
                ui.text(&format!("Active Particles: {}", self.metrics.active_particle_count));
                ui.text(&format!("Total Particles: {}", self.metrics.particle_count));
                ui.text(&format!("Free Particle Pool: {}", self.free_particle_indices.len()));
                ...
                ui.text(&format!("  Update Time: {:.2}ms", self.metrics.get_avg_time(&self.metrics.update_times) * 1000.0));
                ui.text(&format!("  Force Calc: {:.2}ms", self.metrics.get_avg_time(&self.metrics.force_calculation_times) * 1000.0));
                ui.text(&format!("  Integration: {:.2}ms", self.metrics.get_avg_time(&self.metrics.integration_times) * 1000.0));
                ui.text(&format!("  Scene Sync: {:.2}ms", self.metrics.get_avg_time(&self.metrics.sync_times) * 1000.0));
                ui.text(&format!("  Particles/sec: {:.0}", self.metrics.get_particles_per_second()));
                ui.spacing();
        ...
        ui.window("Performance Analysis")
            ...
            .build(|| {
                ...
                ui.text("Statistics:");
                ui.text(&format!("Total Force Calculations: {}", self.metrics.total_force_calculations));
                ui.text(&format!("Total Integrations: {}", self.metrics.total_integrations));
                ui.text(&format!("Memory Accesses: {}", self.metrics.total_memory_accesses));
                ui.spacing();
                
                ui.text("Resource Utilization:");
                ui.text(&format!("Thread Count: {}", self.thread_count));
                ui.text(&format!("Multithreading: {}", if self.use_multithreading { "Enabled" } else { "Disabled" }));
                ui.text(&format!("Memory Pool: {}/{} particles", 
                    self.particles.len(), 
        ...
        if self.show_optimization_panel {
            ui.window("Advanced Optimization")
                ...
                .build(|| {
                    ...
                    ui.text(&format!("Particle Pool Size: {}", self.particle_pool.capacity()));
                    ui.text(&format!("Free Indices: {}", self.free_particle_indices.len()));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create materials for CPU particles
    haggis
        .app_state
        .scene
