...
use haggis::{CutPlane2D, simulation::BaseSimulation};
use cgmath::Vector3;
...
impl ConwaysGpuSimulation {
    fn new() -> Self {
        ...
        println!("🔬 Initial {} pattern: {} live cells", simulation.current_pattern.as_str(), live_count);
    ...
    fn upload_grid_to_gpu(&self, _device: &Device, queue: &Queue) {
        if let Some(ref gpu_resources) = self.gpu_resources {
            ...
            let u32_data: Vec<u32> = self.cpu_grid.iter()
            ...
            if self.generation == 0 {
                ...
                println!("📡 Uploading initial pattern to GPU: {} live cells", live_count);
            }
    ...
    fn sync_gpu_to_cpu_and_viz(&mut self, device: &Device, queue: &Queue) {
        if let Some(ref gpu_resources) = self.gpu_resources {
            ...
            if let Ok(Ok(())) = rx.recv() {
                ...
                let viz_data: Vec<f32> = u32_data.iter()
                ...
                //     println!("🔬 Generation {}: {} live cells", self.generation, live_count);
                // }
                
                // Update visualization
                let mut data_plane = CutPlane2D::new();
                data_plane.update_data(viz_data, self.width, self.height);
                data_plane.set_position(Vector3::new(0.0, 2.0, 0.0));
                data_plane.set_size(2.0);
    ...
    fn render_ui(&mut self, ui: &imgui::Ui) {
        ui.window("Conway's Game of Life GPU")
            ...
            .build(|| {
                ...
                // Pattern selection
                ui.text("Pattern:");
                let patterns = [LifePattern::Random, LifePattern::Glider, LifePattern::Blinker, LifePattern::GosperGun, LifePattern::Clear];
                ...
                if ui.slider_config("Generations/sec", 0.1, 60.0)
